from Routers.History import History
from Routers.Base import BaseCalculater, Baseframe
from Routers.StandardCalculator import StandardCalculator
from Routers.ScientificCalculator import ScientificCalculator
from Routers.ProgrammingCalculator import ProgrammingCalculator
from Routers.Convertor import Currency_convertor, Unit_convertor
from Routers.Pannel import Pannel